...
use crate::clocks::{phclk, Stm32mp157xClocks};
...
struct GpioRegisters {
    ...
    // NOTE: The following registers are included for completeness,
    // but need to have bitfields added (todo).
    /// GPIO port bit reset register
    brr: WriteOnly<u32>,
    /// GPIO secure configuration register
    seccfgr: ReadWrite<u32>,
    /// GPIO hardware configuration register 10
    hwcfgr10: ReadOnly<u32>,
    /// GPIO hardware configuration register 9
    hwcfgr9: ReadOnly<u32>,
    /// GPIO hardware configuration register 8
    hwcfgr8: ReadOnly<u32>,
    /// GPIO hardware configuration register 7
    hwcfgr7: ReadOnly<u32>,
    /// GPIO hardware configuration register 6
    hwcfgr6: ReadOnly<u32>,
    /// GPIO hardware configuration register 5
    hwcfgr5: ReadOnly<u32>,
    /// GPIO hardware configuration register 4
    hwcfgr4: ReadOnly<u32>,
    /// GPIO hardware configuration register 3
    hwcfgr3: ReadOnly<u32>,
    /// GPIO hardware configuration register 2
    hwcfgr2: ReadOnly<u32>,
    /// GPIO hardware configuration register 1
    hwcfgr1: ReadOnly<u32>,
    /// GPIO hardware configuration register 0
    hwcfgr0: ReadOnly<u32>,
...
// (todo) uncomment these once we add these gpio ports. memory mapping
// has already been done here/is checked.
// const GPIOZ_BASE: StaticRef<GpioRegisters> =
//     unsafe { StaticRef::new(0x54004400 as *const GpioRegisters) };
//
// const GPIOK_BASE: StaticRef<GpioRegisters> =
//     unsafe { StaticRef::new(0x5000C000 as *const GpioRegisters) };
//
// const GPIOJ_BASE: StaticRef<GpioRegisters> =
//     unsafe { StaticRef::new(0x5000B000 as *const GpioRegisters) };
//
// const GPIOI_BASE: StaticRef<GpioRegisters> =
//     unsafe { StaticRef::new(0x5000A000 as *const GpioRegisters) };
...
const GPIOF_BASE: StaticRef<GpioRegisters> =
    unsafe { StaticRef::new(0x50007000 as *const GpioRegisters) };

const GPIOE_BASE: StaticRef<GpioRegisters> =
    unsafe { StaticRef::new(0x50006000 as *const GpioRegisters) };
...
const GPIOC_BASE: StaticRef<GpioRegisters> =
    unsafe { StaticRef::new(0x50004000 as *const GpioRegisters) };
...
/// STM32F446RE has 12 GPIO ports labeled from A-K and Z. This is represented
/// by four bits.
#[repr(u32)]
pub enum PortId {
    A = 0b0000,
...
macro_rules! declare_gpio_pins_without_exti {
    ($($pin:ident)*) => {
        [
...
// macro_rules! declare_gpio_pins_with_exti {
//     ($($pin:ident)*, $exti:expr) => {
//         [
...
impl<'a> GpioPorts<'a> {
    pub fn new(clocks: &'a dyn Stm32mp157xClocks) -> Self {
        Self {
            ports: [
                Port {
            ...
            pins: [
                declare_gpio_pins_without_exti! {
...
impl<'a> hil::gpio::Interrupt<'a> for Pin<'a> {
    fn enable_interrupts(&self, _mode: hil::gpio::InterruptEdge) {
        unimplemented!("Interrupts not yet implemented for GPIO pins");
    }
